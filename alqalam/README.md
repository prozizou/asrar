...
- `main.js` — UI + intégration abonnement (gate via `checkAccess` global,
  portail d'offres via `showSubscriptionGate`).
...
- non abonné → ouverture du **portail d'offres** (`sub_*`) ; le choix d'une offre
  ouvre WhatsApp (`ASRAR_WA.openAccess`) pour une activation manuelle par l'admin.
