...
export default function AlQalamPage() {
  ...
  const [souratesStatus, setSouratesStatus] = useState<'loading' | 'ready' | 'error'>('loading');
  ...
  const refreshSourates = () => {
    setSouratesStatus('loading');
    loadSourates((cached: any) => {
      setSourates(cached.list);
      setSouratesContent(cached.content);
      setSouratesStatus('ready');
    }).then((s: any) => {
      if (s.list.length) {
        setSourates(s.list);
        setSouratesContent(s.content);
        setSouratesStatus('ready');
        if (s.offline) showToast('Sourates disponibles depuis le cache local.', 'info');
      } else {
        setSouratesStatus('error');
      }
    }).catch(() => setSouratesStatus('error'));
  };
          ...
          <div className="controls-section">
            ...
            {writingMode === 'intercalee' && (
              <div className="hidden-panel show-panel">
                ...
                >
                  <option value="">{sourates.length ? 'Sélectionnez une sourate' : souratesStatus === 'error' ? 'Sourates indisponibles' : '⏳ Chargement des sourates...'}</option>
                  {sourates.map((s) => (
                ...
                {souratesStatus === 'error' && (
                  <div role="alert" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <span>Impossible de charger les sourates. Vérifiez votre connexion.</span>
                    <button type="button" className="btn-glass-outline" onClick={refreshSourates}>Réessayer</button>
                  </div>
                )}
