...
// Sépare le champ `fr` ("Al-Malik — Le Roi souverain") en transcription et
// description — un seul point de vérité pour ce découpage, repris par
// app/combinaisons/page.tsx (cartes de résultat compactes, fiche de nom) au
// lieu de le refaire à chaque usage (une seule occurrence existait avant,
// dans le tableau des 99 noms — voir revue design du module).
export function splitFr(fr) {
  const i = (fr || '').indexOf('—');
  if (i < 0) return { translit: fr || '', desc: '' };
  return { translit: fr.slice(0, i).trim(), desc: fr.slice(i + 1).trim() };
}
...
export const NAMES = NAMES_DATA.map(([display, clean, fr, precomputed]) => {
  ...
  const weight = Number(precomputed) > 0 ? Number(precomputed) : computed;
