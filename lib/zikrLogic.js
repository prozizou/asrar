...
// Numéro de téléphone (format international) — désormais OBLIGATOIRE pour
// demander à rejoindre un zikr collectif (handleJoin, pages/api/zikr.js),
// afin que le créateur puisse contacter chaque participant par WhatsApp.
// Bornes E.164 : indicatif pays + numéro nationale, 8 à 15 chiffres.
...
/** Normalise ET valide un numéro de téléphone au format international
 * (E.164) : un "+" suivi de 8 à 15 chiffres (indicatif pays inclus).
 * Tolère en saisie les espaces/tirets/points/parenthèses et un préfixe
 * "00" (équivalent courant à "+", ex. "0033612345678") — tout le reste (une
 * saisie sans "+" ni "00", un numéro local sans indicatif) est REJETÉ :
 * mieux vaut demander explicitement le format international que deviner un
 * indicatif pays à la place de l'utilisateur. Retourne le numéro normalisé
 * ("+33612345678") ou null si invalide/absent. Pure. */
export function normalizePhone(v) {
  let s = String(v == null ? '' : v).trim();
  if (!s) return null;
  if (s.startsWith('00')) s = '+' + s.slice(2);
  const m = s.replace(/[^\d+]/g, '').match(new RegExp(`^\\+(\\d{${PHONE_MIN_DIGITS},${PHONE_MAX_DIGITS}})$`));
  return m ? '+' + m[1] : null;
}
...
/** Ce qu'il reste à réciter (jamais négatif) — SOURCE UNIQUE de ce calcul,
 * partagée par CollectiveProgress et MemberCounter (app/zikr/page.tsx) :
 * avant, chacun refaisait `Math.max(0, target - total)` de son côté sur les
 * mêmes valeurs — même résultat en pratique, mais deux endroits à maintenir
 * en cohérence plutôt qu'un seul (revue design : « l'ensemble doit être
 * piloté par une seule source de calcul pour éviter toute divergence
 * future »). Pure. */
export function remainingOf(total, target) {
  return Math.max(0, (Number(target) || 0) - (Number(total) || 0));
}
...
// ── Discussion : identité affichée, avatar, regroupement des messages ──────
// Revue design (« email trop long/répétitif, pas d'avatar ») : ce qui suit
// n'affecte QUE l'affichage — l'email reste la SEULE identité qui fait
// autorité partout ailleurs (RTDB, modération, contact WhatsApp du marché).

/** Nom affiché dans la discussion : le nom Google (server/access.js
 * verifyUser, absent pour un compte email/mot de passe) si connu, sinon la
 * partie locale de l'email — jamais l'email complet, long et répétitif une
 * fois affiché sur chaque message. Pure. */
export function chatDisplayName(email, name) {
  if (name && String(name).trim()) return String(name).trim();
  const e = String(email == null ? '' : email);
  const at = e.indexOf('@');
  return at > 0 ? e.slice(0, at) : e || 'Membre';
}

// Palette FIXE (pas aléatoire) pour l'avatar-initiale d'un membre sans photo
// Google — cohérente avec l'identité violette de l'app plutôt qu'une couleur
// tirée au hasard à chaque rendu.
export const AVATAR_COLORS = ['#a78bfa', '#f472b6', '#38d66b', '#60a5fa', '#fbbf24', '#f87171'];

/** Couleur d'avatar STABLE pour un uid donné (même compte → même couleur à
 * chaque rendu, sur tous les appareils) — hash simple, déterministe, pas
 * besoin de cryptographique ici. Pure. */
...
// Fenêtre de regroupement des messages consécutifs d'un même auteur (façon
// WhatsApp/Telegram) : un écart plus grand redémarre un groupe (avec son
// propre en-tête horodaté) même si c'est le même auteur — un silence de
// plusieurs minutes n'est plus "le même instant de conversation".
export const CHAT_GROUP_WINDOW_MS = 5 * 60 * 1000;

/** Regroupe des messages CONSÉCUTIFS du même auteur (voir CHAT_GROUP_WINDOW_MS)
 * — demandé explicitement (« l'identité est répétée inutilement ») : un seul
 * en-tête (avatar/nom/heure) par groupe plutôt qu'à chaque message. Pure —
 * ne réordonne rien, `messages` doit déjà être trié par `at` croissant
 * (comme le renvoie déjà pages/api/zikr.js handleMessages).
 * @param {{uid:string, at:number}[]} messages
 * @returns {{uid:string, items:object[]}[]}
 */
export function groupChatMessages(messages) {
  const groups = [];
  for (const m of messages || []) {
    const last = groups[groups.length - 1];
    const prevAt = last ? last.items[last.items.length - 1].at : null;
    if (last && last.uid === m.uid && prevAt !== null && m.at - prevAt < CHAT_GROUP_WINDOW_MS) {
      last.items.push(m);
    } else {
      groups.push({ uid: m.uid, items: [m] });
    }
  }
  return groups;
}
