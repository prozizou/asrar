...
describe('diamond8 — losange magique', () => {
  ...
  it('diamond8(903) — base hors de la forme 124 + 8n : les 10 rangées/colonnes somment quand même exactement à 903', () => {
    // isValidDiamond8Base(903) est false ((903 - 124) / 8 = 97,375) : le
    // correctif (voir le commentaire de DIAMOND8_REMAINDER_FIGURES dans
    // kanzouWafq.ts) doit malgré tout donner des sommes exactes.
    expect(isValidDiamond8Base(903)).toBe(false);
    const d = diamond8(903);
    const byPQ = new Map(d.cells.map((c) => [`${c.p},${c.q}`, c]));
    const cell = (p, q) => byPQ.get(`${p},${q}`);
    const pRow = (p) => [0, 1, 2, 3].flatMap((q) => [cell(p, q).outer, cell(p, q).inner]);
    const qRow = (q) => [0, 1, 2, 3].flatMap((p) => [cell(p, q).outer, cell(p, q).inner]);
    const diagMain = [0, 1, 2, 3].flatMap((i) => [cell(i, i).outer, cell(i, i).inner]);
    const diagAnti = [0, 1, 2, 3].flatMap((p) => [cell(p, 3 - p).outer, cell(p, 3 - p).inner]);
    const sum = (arr) => arr.reduce((a, b) => a + b, 0);

    for (let p = 0; p < 4; p++) expect(sum(pRow(p))).toBe(903);
    for (let q = 0; q < 4; q++) expect(sum(qRow(q))).toBe(903);
    expect(sum(diagMain)).toBe(903);
    expect(sum(diagAnti)).toBe(903);
  });
});
