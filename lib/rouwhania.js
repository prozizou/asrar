...
export function findAllahNameByLetter(char, userWish, asmaData) {
  ...
  // Bug distinct (et probablement la cause principale du problème signalé) :
  // sans vœu saisi (champ souvent laissé vide), `keywords` est vide → aucun
  // candidat ne peut jamais dépasser MIN_SCORE, donc la fonction renvoyait
  // systématiquement null MÊME QUAND un nom correspond bel et bien à la
  // lettre (matchingNames non vide) — « aucun nom adapté » alors qu'il en
  // existe. Le score ne doit départager QUE lorsqu'un vœu a été fourni ;
  // sans vœu, on renvoie simplement le premier nom qui correspond à la
  // lettre plutôt que de bloquer faute de signal à noter.
  const MIN_SCORE = 1;
  let bestMatch = matchingNames[0];
  let maxScore = -1;
  ...
  // Vœu fourni mais aucun candidat n'y correspond vraiment → null a du sens
  // ici. Sans vœu (keywords vide), on garde le repli sur matchingNames[0].
  if (keywords.length > 0 && maxScore < MIN_SCORE) return null;
  return bestMatch;
}
