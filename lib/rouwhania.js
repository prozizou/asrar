...
// benefits.js loadNames()), avec repli sur une lecture Firestore directe,
// chargée à la demande (Phase 3 de la migration RTDB → Firestore, voir
// docs/FIRESTORE_SCHEMA.md et le commentaire de getFirestoreLazy dans
// lib/firebase.js). Avant la correction d'origine, loadRouwhaniaAsma()/
// loadVerses() ne faisaient QUE la lecture directe (SDK modulaire) — sur un
// réseau où ce canal reste bloqué en silence (cf. l'historique
// /api/check-access, /api/social), asma/verses restaient vides SANS AUCUNE
// erreur visible : les cartes « Les noms d'Allah qui vont avec » affichaient
// alors systématiquement « aucun nom adapté » pour TOUTES les lettres (asma
// vide → aucun candidat possible), et aucune suggestion de verset ne
// s'affichait jamais.
...
// affiche en pleine largeur sous chaque lettre).
...
export async function loadVerses() {
  ...
  // 3) Lecture Firestore directe (repli)
  try {
    ...
    snap.forEach((doc) => {
      const item = doc.data();
      if (item && item.verset) out.push({ id: doc.id, key: item.key || '', verset: item.verset.trim() });
    });
    return out;
  } catch {
    return [];
