...
export function sunTimesFor(dateLocal, lat, lng) {
  ...
  const lw = -lng;
  // Longitude ouest positive : le midi solaire arrive plus tard en UTC.
...
// Source de production unique : calcul solaire local, identique côté client/serveur.
// Le cache injecté reste disponible pour des données déterministes (tests).
// Sans position ou en zone polaire : défaut 06:30/18:00.
...
// Date civile dans le fuseau de l'appareil, même lorsque le serveur est en UTC.
...
export function computePday(now, lat, lng, cache, timeZone = undefined) {
  const date = civilDate(now, timeZone);
  const today = sunOrDefault(date, lat, lng, cache);
  if (now.getTime() < today.sunrise.getTime()) {
    const veille = new Date(date);
    veille.setDate(veille.getDate() - 1);
    const y = sunOrDefault(veille, lat, lng, cache);
    return { sunrise: y.sunrise, sunset: y.sunset, nextSunrise: today.sunrise, dayOfWeek: veille.getDay(), today };
  }
  const demain = new Date(date);
  demain.setDate(demain.getDate() + 1);
  const t = sunOrDefault(demain, lat, lng, cache);
  return { sunrise: today.sunrise, sunset: today.sunset, nextSunrise: t.sunrise, dayOfWeek: date.getDay(), today };
}
...
// État partagé par la page, les push et les alarmes préparées à l'avance.
// Les bornes sont des instants absolus ; aucune conversion de texte horaire.
export function planetaryHourState(hour, at) {
  const startMs = hour.start.getTime();
  const endMs = hour.end.getTime();
  const fraction = (at.getTime() - startMs) / (endMs - startMs);
  const nature = natureOf(hour.planet, fraction);
  const half = hour.planet === 'Mercure' ? (fraction < 0.5 ? 1 : 2) : null;
  const validUntil = new Date(half === 1 ? startMs + (endMs - startMs) / 2 : endMs);
  return { ...hour, fraction, nature, half, validUntil,
    stateKey: `${hour.planet}:${startMs}:${half || 0}` };
}

export function planetaryNotification(hour, at, timeZone = undefined) {
  const state = planetaryHourState(hour, at);
  const until = state.validUntil.toLocaleTimeString('fr-FR', {
    hour: '2-digit', minute: '2-digit', ...(timeZone ? { timeZone } : {}),
  });
  return { state, title: `${CHALDEAN_EMOJIS[hour.planet]} ${hour.planet}`,
    body: `${state.nature.txt} — jusqu’à ${until}` };
}
...
// `start`/`end` (Date, en plus de `interval` déjà formaté) : nécessaires pour
// programmer une alarme locale exacte sur une occurrence précise (lib/
// planetAlarms.js) — `interval` seul (texte) ne permettait pas de retrouver
// l'instant exact visé par un clic sur « aujourd'hui ».
export function buildHourList(pday, cur, isDay) {
  ...
  for (let k = 0; k < 12; k++) {
    ...
    rows.push({
      interval: `${fmtHM(s)} – ${fmtHM(e)}`,
      start: new Date(s),
      end: new Date(e),
      planet,
      emoji: CHALDEAN_EMOJIS[planet],
      nat,
      isNow,
    });
  }
...
// Occurrences à VENIR d'UNE planète donnée, sur `days` jours à partir de
// `fromDate` — sert la répétition d'alarme (lib/planetAlarms.js,
// REPEAT_HORIZON_DAYS) : sans service natif en arrière-plan, il faut
// programmer à l'avance toutes les occurrences sur une fenêtre bornée plutôt
// qu'une seule. Réutilise buildHourList (jour ET nuit) pour chaque jour
// solaire de la fenêtre — `noMatch` (jamais égal à `isDay`, qui vaut
// toujours true/false) neutralise `isNow`, sans objet pour une date future.
// midi local comme instant de référence par jour : toujours après le lever
// (sauf cas polaire, déjà couvert par le repli de sunOrDefault), donc sans
// ambiguïté sur la journée planétaire visée quelle que soit l'heure d'appel.
export function upcomingPlanetOccurrences(planet, fromDate, days, lat, lng, cache, nowMs = Date.now()) {
  ...
  for (let d = 0; d < days; d++) {
    ...
    const pday = computePday(ref, lat, lng, cache);
    for (const isDay of [true, false]) {
      for (const row of buildHourList(pday, noMatch, isDay)) {
        if (row.planet === planet && row.start.getTime() > nowMs) occurrences.push(row);
      }
    }
  }
