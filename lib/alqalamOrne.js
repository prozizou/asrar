    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      row.forEach((tok, tokenIndex) => {
        ...
        if (prevTok && prevTok.joinsForward && !tokenStartsWithSpace(tok)) {
          ...
          const gap = Math.max(0, prevTouch - rightTouch);
          const prevOverlap = prevShape.kind === 'text' ? Math.min(textFont * 0.22, gap * 0.45) : Math.min(loopStroke / 2, gap * 0.2);
          const nextOverlap = curShape.kind === 'text' ? Math.min(textFont * 0.22, gap * 0.45) : Math.min(loopStroke / 2, gap * 0.2);
          connectors.push({ x1: prevTouch + prevOverlap, x2: rightTouch - nextOverlap, y: connectorY, width });
        }
