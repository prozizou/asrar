...
export function tokenizePhrase(phrase, letters) {
  ...
  for (const tok of coarse) {
    ...
    for (const piece of pieces) {
      // Les voyelles et la chadda sont transparentes pour la liaison :
      // c'est la dernière lettre de base qui détermine sa forme contextuelle.
      const baseEnd = piece.replace(/\p{M}+$/u, '');
      tokens.push({ type: 'text', value: piece, joinsForward: charJoinsForward(Array.from(baseEnd).pop()) });
    }
  }
...
export const PHRASE_PIECE = {
  ...
  W: 1200,
  H_MAX: 1200 * (210 / 297),
  MARGIN: 1200 * (5 / 297), // 5 mm = 0,5 cm sur les quatre côtés
  FOOTER_HEIGHT: 32, // zone réservée à la pagination, dans les marges
  INK_INSET: 6, // protège les contours et les débords des glyphes
...
export function paginateRows(rowCount, rowHeight, rowGap, maxHeight) {
  ...
  const baseSize = Math.floor(rowCount / pageCount);
  const largerPages = rowCount % pageCount;
  const pages = [];
  for (let i = 0, index = 0; i < pageCount; i += 1) {
    const page = [];
    for (let j = 0; j < baseSize + (i < largerPages ? 1 : 0); j += 1) page.push(index++);
    pages.push(page);
  }
  return pages;
}
 ...
 */
function mergeAdjacentText(row, measure, textFont, preserveWords = false) {
  const merged = [];
  for (const tok of row) {
    const last = merged[merged.length - 1];
    if (tok.type === 'text' && last && last.type === 'text' &&
        (!preserveWords || (!tokenStartsWithSpace(tok) && !/\s$/.test(last.value)))) {
...
function minRadiusForVow(shapes, innerText, measure) {
  ...
  const fits = (radius) => shapes.every((s) => {
    ...
    return !!fitText({ text, a, b, n: s.n, measure, maxFont: radius * 0.32, minFont: 2, step, yMin, yMax });
  });
...
export function composePhrasePages({ phrase, letters, innerText, measure }) {
  ...
  const inset = MARGIN + PHRASE_PIECE.INK_INSET;
  const maxWidth = W - 2 * inset;
  const maxHeight = H_MAX - 2 * inset - PHRASE_PIECE.FOOTER_HEIGHT;

  const tokens = tokenizePhrase(String(phrase || '').trim().replace(/\s+/g, ' '), letters);
  ...
  let radius = Math.min(Math.max(targetRadius, vowRadius), maxHeight / (topRatio + bottomRatio), 2 * PHRASE_PIECE.INK_INSET / 0.045);
  ...
  const pages = kept.map((rowIndices) => {
    ...
    // (typiquement la dernière) : son contenu reste en haut plutôt que
    // de rétrécir la page à ce contenu — demandé explicitement (« que les
    // pages soient en A4 »).
    const H = H_MAX;
    const startY = inset;
    const contentH = rowIndices.length * rowHeight + Math.max(0, rowIndices.length - 1) * rowGap;
    const extraHeight = Math.max(0, maxHeight - contentH);
    // Justification verticale mesurée : la dernière page profite aussi de
    // la hauteur disponible, sans agrandir ou déformer les lettres.
    const pageRowGap = rowIndices.length > 1
      ? rowGap + Math.min(rowHeight * 0.5, extraHeight / (rowIndices.length - 1))
      : rowGap;
    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      const hasLoops = rows[rowIdx].some((tok) => tok.type === 'loop');
      const row = mergeAdjacentText(rows[rowIdx], measure, textFont, hasLoops);
      // Quand une boucle est présente, on décale les MOTS complets : jamais
      // une lettre séparée de sa boucle. Sans boucle, le texte SVG peut
      // justifier ses espaces ordinaires sans changer la forme des glyphes.
      const boundaries = hasLoops
        ? row.reduce((sum, _, i) => sum + (i > 0 && startsNewWord(row, i) ? 1 : 0), 0)
        : row.reduce((sum, tok) => sum + (tok.type === 'text' ? (tok.value.match(/ /g) || []).length : 0), 0);
      const naturalWidth = row.reduce((sum, tok) => sum + tok.w, 0) + Math.max(0, row.length - 1) * radius * GAP_RATIO;
      // Ne pas créer de blancs de plusieurs centimètres lorsqu'une ligne
      // contient peu de mots : elle reste alors alignée à droite.
      const wordSpacing = rowIdx < rows.length - 1 && boundaries > 0
        ? Math.min(radius * 0.22, Math.max(0, maxWidth - naturalWidth) / boundaries) : 0;
      if (!hasLoops) {
        for (const tok of row) {
          if (tok.type === 'text') tok.w += wordSpacing * (tok.value.match(/ /g) || []).length;
        }
      }
      const rowTop = startY + ri * (rowHeight + pageRowGap);
      ...
      row.forEach((tok, tokenIndex) => {
        if (hasLoops && tokenIndex > 0 && startsNewWord(row, tokenIndex)) {
          itemRight -= wordSpacing;
        }
        ...
        //
        // Le canvas mesure la chasse, y compris les blancs de bord des
        // glyphes. Un court recouvrement sous l'encre supprime la rupture
        // visible sans étirer la lettre ni relier deux mots distincts.
        if (prevTok && prevTok.joinsForward && !tokenStartsWithSpace(tok)) {
          let width = prevTok.type === 'loop' ? loopStroke : textStroke;
          // La chasse typographique n'est pas le bord de l'encre. Mesurer
          // le trait de liaison du glyphe déjà formé, dans la police chargée.
          const textShape = prevShape.kind === 'text' ? prevShape : curShape.kind === 'text' ? curShape : null;
          const textToken = prevShape.kind === 'text' ? prevTok : tok;
          const side = prevShape.kind === 'text' ? 'left' : 'right';
          const ink = textShape && measure.connection?.(textToken.value, textFont, side);
          if (ink) {
            connectorY = textY + ink.y;
            width = ink.width;
          }
          const prevTouch = edgeAt(prevShape, 'left');
          const nextTouch = edgeAt(curShape, 'right');
          // Arrêt au centre du contour, sans embout rond ni prolongement
          // dans la boucle. Côté texte, recouvrement limité au trait mesuré.
          const gap = Math.max(0, prevTouch - nextTouch);
          const overlap = Math.min(textFont * 0.22, gap * 0.45);
          const x1 = prevShape.kind === 'text'
            ? ink ? prevShape.left + ink.x : prevTouch + overlap : prevTouch;
          const x2 = curShape.kind === 'text'
            ? ink ? curShape.left + ink.x : nextTouch - overlap : nextTouch;
          connectors.push({ x1, x2, y: connectorY, width });
        }
        ...
        if (tok.type === 'text') {
          items.push({
            ...
            y: textY,
            fontSize: textFont,
            wordSpacing: hasLoops ? 0 : wordSpacing,
          });
        } else {
          ...
          const inner = hasInner
            ? fitText({
                ...
                maxFont: radius * 0.32, minFont: 2, step: 0.35,
...
export function makeMeasurer(fontFamily, weight = 700) {
  ...
  // Comme le SVG : en LTR, un ZWJ final peut sortir du segment arabe et
  // la mesure porter sur une lettre finale/isolée au lieu de la forme liée.
  ctx.direction = 'rtl';
  ...
  const cache = new Map();
  measure.connection = (text, fontSize, side) => {
    const key = `${fontSize}:${side}:${text}`;
    if (cache.has(key)) return cache.get(key);
    const scale = 4;
    const pad = fontSize;
    const raster = document.createElement('canvas');
    raster.width = Math.ceil((measure(text, fontSize) + 2 * pad) * scale);
    raster.height = Math.ceil(3 * fontSize * scale);
    const ink = raster.getContext('2d', { willReadFrequently: true });
    ink.scale(scale, scale);
    ink.font = `${weight} ${fontSize}px ${fontFamily}`;
    ink.direction = 'rtl';
    ink.textAlign = 'left';
    ink.fillText(text, pad, 2 * fontSize);
    const { data } = ink.getImageData(0, 0, raster.width, raster.height);
    // Le raccord est proche de la ligne de base ; exclure points et accents.
    const top = Math.floor(1.72 * fontSize * scale);
    const bottom = Math.ceil(2.06 * fontSize * scale);
    const runsAt = (x) => {
      const runs = [];
      let start = -1;
      for (let y = top; y <= bottom; y++) {
        const black = y < bottom && data[(y * raster.width + x) * 4 + 3] >= 128;
        if (black && start < 0) start = y;
        if (!black && start >= 0) { runs.push({ top: start, bottom: y }); start = -1; }
      }
      return runs.sort((a, b) => (b.bottom - b.top) - (a.bottom - a.top))[0];
    };
    const step = side === 'left' ? 1 : -1;
    let edge = side === 'left' ? 0 : raster.width - 1;
    while (edge >= 0 && edge < raster.width && !runsAt(edge)) edge += step;
    // Lire légèrement à l'intérieur du glyphe pour éviter son antialiasing.
    const x = edge + step * Math.max(1, Math.round(fontSize * scale * 0.025));
    const run = x >= 0 && x < raster.width ? runsAt(x) : null;
    const result = run ? {
      x: x / scale - pad,
      y: (run.top + run.bottom) / (2 * scale) - 2 * fontSize,
      width: (run.bottom - run.top) / scale,
    } : null;
...
// ── Export PDF indépendant de l'orientation du dialogue Android ────────────
/**
 * Convertit chaque SVG déjà composé en une page A4 paysage. Les caractères
 * arabes sont dessinés par le même moteur Canvas que le mesureur de lignes ;
 * aucun réglage d'impression du téléphone ne peut revenir au portrait.
 */
export async function printPieces(container, docName) {
  if (!container) throw new Error('Aucune pièce à exporter.');
  if (container.dataset.exportReady === 'false') {
    const reason = container.dataset.exportReason;
    if (reason === 'vow') throw new Error('Le vœu est trop long pour les boucles. Raccourcissez-le avant l’export PDF.');
    if (reason === 'pages') throw new Error(`Le texte dépasse ${PHRASE_PIECE.MAX_PAGES} pages. Réduisez les répétitions avant l’export PDF.`);
    throw new Error('Un mot est trop large pour une page A4. Raccourcissez-le avant l’export PDF.');
  }
  const svgs = Array.from(container.querySelectorAll('svg.orne-svg'));
  if (!svgs.length) throw new Error('Aucune pièce à exporter.');
  if (container.dataset.exporting === 'true') throw new Error('Un export PDF est déjà en cours.');
  container.dataset.exporting = 'true';

  try {
    await ensureFontReady('700 40px "Scheherazade New"');
    const [{ Canvg }, { jsPDF }] = await Promise.all([import('canvg'), import('jspdf')]);
    const pdf = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4', compress: true });
    // Une seule surface réutilisée pour toutes les feuilles. Pour un ouvrage
    // long, limiter la résolution évite d'épuiser la mémoire d'un téléphone.
    const scale = svgs.length > 20 ? 1.7 : 2.5;
    const canvas = document.createElement('canvas');
    canvas.width = Math.round(297 * 96 / 25.4 * scale);
    canvas.height = Math.round(210 * 96 / 25.4 * scale);
    const ctx = canvas.getContext('2d', { alpha: false });
    if (!ctx) throw new Error('Le rendu PDF est indisponible sur cet appareil.');
    // Canvg 3 ne transmet pas `direction: rtl` du SVG au Canvas. Il faut
    // préserver le contexte bidi pour que les ZWJ de bord lient les lettres.
    // Canvg calcule déjà l'ancrage SVG : ses coordonnées sont le bord gauche,
    // donc ne pas laisser l'alignement Canvas `start` devenir le bord droit.
    ctx.direction = 'rtl';
    ctx.textAlign = 'left';
    ...
    for (let i = 0; i < svgs.length; i += 1) {
      ...
      // Canvg élague les espaces de bord malgré xml:space="preserve".
      // Les garder avec un repère RTL sans chasse évite de décaler le texte
      // d'un demi-espace par rapport aux raccords calculés pour l'aperçu.
      clone.querySelectorAll('text[xml\\:space="preserve"]').forEach((text) => {
        text.textContent = text.textContent.replace(/^\s/, '\u200f$&').replace(/\s$/, '$&\u200f');
      });
      ctx.fillStyle = '#fff';
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      const renderer = await Canvg.fromString(ctx, new XMLSerializer().serializeToString(clone), {
        // Le canvas a déjà ses dimensions : le redimensionner réinitialise
        // direction/textAlign et fait réapparaître les formes isolées.
        ignoreDimensions: true, ignoreAnimation: true, ignoreMouse: true,
      });
