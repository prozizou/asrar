...
export function composePhrasePages({ phrase, letters, innerText, measure }) {
  ...
  const pages = kept.map((rowIndices) => {
    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      // Espaces inter-mots seulement : aucun scaleX, textLength ou étirement
      // des glyphes. La dernière ligne reste naturellement alignée à droite.
      const spaces = row.reduce((sum, tok) => sum + (tok.type === 'text' ? (tok.value.match(/ /g) || []).length : 0), 0);
      const naturalWidth = row.reduce((sum, tok) => sum + tok.w, 0) + Math.max(0, row.length - 1) * radius * GAP_RATIO;
      const wordSpacing = rowIdx < rows.length - 1 && spaces > 0
        ? Math.max(0, maxWidth - naturalWidth) / spaces : 0;
      for (const tok of row) {
...
export async function printPieces(container, docName) {
  ...
  if (container.dataset.exportReady === 'false') {
    throw new Error('La composition est incomplète. Corrigez le texte ou le vœu avant d’exporter.');
  }
  const svgs = Array.from(container.querySelectorAll('svg.orne-svg'));
  if (!svgs.length) throw new Error('Aucune pièce à imprimer.');
  if (document.getElementById(ORNE_PAGE_STYLE_ID)) {
  ...
  // Une feuille physique par SVG, sans marge @page supplémentaire : les
  // 5 mm sont déjà réservés par la composition. Les dimensions fixes en mm
  // évitent les pages parasites dues aux arrondis du ratio et au style de l'app.
  pageStyle.textContent = `
    @media print {
      ...
      #${PDF_PRINT_ROOT_ID} { display: block !important; width: 297mm; margin: 0 !important; padding: 0 !important; }
      ...
      #${PDF_PRINT_ROOT_ID} svg { display: block; width: 297mm; height: 210mm; max-width: none; border: 0; border-radius: 0; }
    }
    @media screen {
