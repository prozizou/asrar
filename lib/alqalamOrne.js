  ...
  const pages = kept.map((rowIndices) => {
    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      row.forEach((tok) => {
        ...
        // Bords RÉELS de CE jeton à la hauteur textY (voir remarque
        // ci-dessus) — calculés AVANT le connecteur, qui a besoin du bord
        // droit, et réutilisés pour prevTouch (bord gauche) en fin de tour.
        let rightTouch = itemRight;
        let leftTouch = itemLeft;
        let norm = null;
        let cx = null;
        if (tok.type === 'loop') {
          norm = shapeFor(tok.letter, radius);
          cx = itemRight - norm.a;
          const halfAtTextY = halfWidthAt(textY - cy, norm.a, norm.b, norm.n);
          rightTouch = cx + halfAtTextY;
          leftTouch = cx - halfAtTextY;
        }
        ...
        if (prevTouch !== null && prevTok.joinsForward && !tokenStartsWithSpace(tok)) {
          const width = prevTok.type === 'loop' ? loopStroke : textStroke;
          // Chevauchement volontaire de chaque côté : la mesure d'un texte
          // (avance de la police) ne correspond pas toujours exactement à
          // l'encre visible du glyphe (empattement latéral propre à la
          // police) — un trait qui s'arrête pile à la mesure théorique
          // pouvait donc laisser un vide visible avant la lettre. Empiéter
          // légèrement DANS les deux voisins garantit qu'il n'y a jamais
          // d'espace, quitte à un tout petit recouvrement — signalé au zoom.
          const overlap = width * 1.5;
          connectors.push({ x1: prevTouch + overlap, x2: rightTouch - overlap, y: textY, width });
        }
...
export async function printPieces(container, docName) {
  ...
  // PIÈGE ÉVITÉ ICI (signalé : PDF avec énormément de marge, la pièce
  // minuscule et centrée sur une page presque vide) : la version précédente
  // calait chaque page sur `height: 100vh` dans un conteneur flex centré.
  // En impression — SURTOUT sur les moteurs mobiles (le cas signalé) —
  // `vh` ne correspond pas de façon fiable à la hauteur de la page
  // imprimée ; le flex-centrage réduisait alors le SVG à une fraction de
  // la page, avec tout l'espace restant en marge morte. Remplacé par un
  // flux BLOC ordinaire (comme le fait déjà l'export texte, lib/alqalam.js,
  // qui n'a jamais eu ce problème) : le SVG occupe 100% de la largeur
  // imprimable, sa hauteur suit son propre ratio — qui EST exactement celui
  // de la page (PHRASE_PIECE.W/H_MAX = 297/210, voir plus haut), donc sans
  // distorsion. La marge @page est réduite au minimum de sécurité
  // imprimante : la pièce a déjà sa PROPRE marge interne (PHRASE_PIECE.MARGIN,
  // en unités SVG) — une seconde marge @page par-dessus ne ferait
  // qu'épaissir le cadre blanc pour rien.
