 ...
 * NON_FORWARD_JOINING) ou pour une boucle issue d'un ة — c'est ce champ qui
 * permet à composePhrasePages de NE PAS tracer de tatweel là où aucune
 * liaison réelle n'existe.
 * @param {string} phrase
...
export function tokenizePhrase(phrase, letters) {
  ...
  const tokens = [];
  let buf = '';
  const bufJoinsForward = () => !NON_FORWARD_JOINING.has(Array.from(buf).pop());
  for (const ch of Array.from(String(phrase || ''))) {
    const norm = EQUIV[ch] || ch;
    if (set.has(norm)) {
      if (buf) tokens.push({ type: 'text', value: buf, joinsForward: bufJoinsForward() });
      buf = '';
      tokens.push({ type: 'loop', letter: norm, joinsForward: !NON_FORWARD_JOINING.has(ch) });
    } else {
      buf += ch;
    }
  }
  ...
  // fait alors que suivre le VŒU inverse de ce qui doit se passer.
