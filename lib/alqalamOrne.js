...
// ronde — م ه ص ض ط — et on écrit à l'intérieur une invocation ou un verset
// en micro-écriture.
//
...
export const ROUND_LETTERS = [
  ...
  // la demande : sa panse ne se prête pas au rond plein qu'offrent les
  // cinq lettres ci-dessus. ف et و n'ont jamais fait partie de cette liste.
];
...
export function tokenizePhrase(phrase, letters) {
  ...
  // JAMAIS si le voisin ne joint justement pas vers l'avant : le liant ne
  // ferait alors que contredire ce qui doit se passer. Un morceau de mot
  // intermédiaire (ni premier ni dernier de son segment d'origine) ne
  // touche jamais directement une boucle : aucune des deux conditions
  // ci-dessous ne s'y applique jamais.
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (prev && prev.type === 'loop' && prev.joinsForward) value = JOIN + value;
...
function startsNewWord(tokens, i) {
  ...
  // `cur.value` peut commencer par le liant JOIN (ZWJ, voir tokenizePhrase)
  // AVANT le véritable espace — un ZWJ n'est pas de l'espace, mais il ne
  // doit pas non plus masquer l'espace réel qui le suit : `‍?` l'avale sans
  // conséquence s'il est présent.
  if (cur.type === 'text' && /^‍?\s/.test(cur.value)) return true;
...
export function composePhrasePages({ phrase, letters, innerText, measure }) {
  ...
  const pages = kept.map((rowIndices) => {
    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      // du jeton suivant plutôt que dans l'espace qui les sépare. prevTok
      // porte le jeton lui-même (pas seulement sa position) : c'est lui qui
      // dit si CE jeton joint vers l'avant (prevTok.joinsForward, voir
      // tokenizePhrase/NON_FORWARD_JOINING) et quelle épaisseur donner au
      // tatweel — la sienne, jamais une valeur fixe.
      ...
      row.forEach((tok) => {
        ...
        if (prevLeft !== null && prevTok.joinsForward) {
          const width = prevTok.type === 'loop' ? loopStroke : textStroke;
          connectors.push({ x1: itemRight, x2: prevLeft, y: textY, width });
        }
