...
export const ROUND_LETTERS = [
  ...
  // un ARC DE CERCLE (bowlPath ouvert, voir ARC_SPAN), PAS un rond plein
  // comme م/ه : leur panse réelle est un croissant ouvert, jamais une
...
// (ص ض ط ظ, voir ROUND_LETTERS) : un croissant ouvert, pas un rond plein
// comme م/ه — demandé explicitement. Le trou est centré en BAS (voir
// bowlPath) : c'est là, loin du texte casé au centre par fitText, qu'il ne
// gêne jamais le calage (halfWidthAt continue de traiter la forme comme
// pleine pour le texte — le trou n'entame que le dernier dixième du bas).
export const ARC_SPAN = Math.PI * 2 * (310 / 360);

/**
 * Contour de la boucle, centré sur (0,0). Fermé (rond plein) par défaut ;
 * avec `arcSpan` < 2π (voir ARC_SPAN), un ARC OUVERT — un croissant, pas un
 * rond — avec le trou centré en bas de la forme.
 */
export function bowlPath(a, b, n, steps = 240, arcSpan = Math.PI * 2) {
  const e = 2 / n;
  const closed = arcSpan >= Math.PI * 2 - 1e-9;
  const gap = Math.PI * 2 - arcSpan;
  // t=π/2 (sin=1, y=+b) est le bas de la forme en SVG (y croît vers le bas) :
  // le tracé démarre juste après le trou et en fait tout le tour jusqu'à
  // juste avant, dans le sens croissant.
  const start = closed ? 0 : Math.PI / 2 + gap / 2;
  const count = closed ? steps : Math.max(8, Math.round(steps * (arcSpan / (Math.PI * 2))));
  const pts = [];
  for (let i = 0; i <= count; i++) {
    if (closed && i === count) break; // comportement historique : pas de point dupliqué, 'Z' referme
    const t = start + (closed ? (i / steps) * Math.PI * 2 : (i / count) * arcSpan);
    const c = Math.cos(t);
    const s = Math.sin(t);
...
function minRadiusForVow(shapes, innerText, measure) {
  ...
  const fits = (radius) => shapes.every((s) => {
    ...
    return !!fitText({ text, a, b, n: s.n, measure, maxFont: radius * 0.32, minFont: 3, step });
  });
    ...
    rowIndices.forEach((rowIdx, ri) => {
      ...
      row.forEach((tok) => {
        ...
        if (prevTouch !== null && prevTok.joinsForward && !tokenStartsWithSpace(tok)) {
          ...
          connectors.push({ x1: prevTouch, x2: rightTouch, y: textY, width });
        }
