...
export function stripLetterOccurrences(word, letter) {
  ...
  const isTarget = (c) => (EQUIV[c] || c) === letter;
  let out = '';
  let i = 0;
  while (i < chars.length) {
    ...
    // Consomme toute la plage retirée d'un coup (occurrences consécutives,
    // ex. 'مم') : un seul liant remplace toute la plage, pas un par lettre.
    while (i < chars.length && isTarget(chars[i])) i += 1;
    // Aucun liant en tout début ou toute fin de mot : rien n'y attend de
    // continuité de ce côté-là (voir le commentaire de JOIN plus haut).
    if (out.length && i < chars.length) out += JOIN;
  }
  return out;
}
...
// Pratique DISTINCTE de composePiece ci-dessus : au lieu d'UN mot avec SA
// lettre gonflée en une boucle dominant toute la page, on écrit une PHRASE où
// CHAQUE occurrence d'une ou plusieurs lettres choisies devient sa propre
// boucle — ex. « بسم الله الرحمن الرحيم » avec م et ه gonflés donne 4 boucles
// (بس-م, الل-ه, الرح-م-ن, ...-حي-م), le même vœu répété dans chacune.
// Fonctions séparées plutôt qu'une généralisation de composePiece : ce dernier
// est déjà en production (une seule boucle dominante, testée, approuvée) —
// mieux vaut AJOUTER cette pratique à côté que de risquer de le déstabiliser.
...
export function tokenizePhrase(phrase, letters) {
  ...
  // Même correctif que stripLetterOccurrences, ici entre jetons plutôt qu'à
  // l'intérieur d'un mot : un segment de texte collé à une boucle doit
  // percevoir cette boucle comme une lettre qui joint, sous peine de voir sa
  // lettre de bord retomber sur sa forme isolée/finale (ex. le س de « بس »,
  // premier segment de « بسم الله… » avec م gonflé).
