...
describe('computePday (journée planétaire : lever → lever)', () => {
  ...
  it('avant le lever du jour, reste rattaché à la journée de la veille', () => {
    ...
    const pday = computePday(now, lat, lng, null, 'Africa/Dakar');
    // La journée en cours doit avoir commencé la veille.
    expect(pday.sunrise.getUTCDate()).toBe(date.getDate() - 1);
  });
  ...
  it('après le lever, la journée planétaire est celle du jour même', () => {
    ...
    const pday = computePday(now, lat, lng, null, 'Africa/Dakar');
    expect(pday.sunrise.getUTCDate()).toBe(date.getDate());
...
describe('upcomingPlanetOccurrences', () => {
  // Dakar (14.7°N) — même fixture que sunTimesFor ci-dessus.
  const LAT = 14.7, LNG = -17.4;

  it('chaque planète apparaît 3 ou 4 fois par jour (24 heures / 7 planètes)', () => {
    const cache = {};
    const from = new Date(2026, 5, 21);
    const occurrences = upcomingPlanetOccurrences('Jupiter', from, 7, LAT, LNG, cache, from.getTime() - 1);
    expect(occurrences.length).toBeGreaterThanOrEqual(3 * 7);
    expect(occurrences.length).toBeLessThanOrEqual(4 * 7);
  });

  it('les occurrences renvoyées sont strictement croissantes dans le temps', () => {
    const cache = {};
    const from = new Date(2026, 5, 21);
    const occurrences = upcomingPlanetOccurrences('Saturne', from, 5, LAT, LNG, cache, from.getTime() - 1);
    for (let i = 1; i < occurrences.length; i++) {
      expect(occurrences[i].start.getTime()).toBeGreaterThan(occurrences[i - 1].start.getTime());
    }
  });

  it('exclut les occurrences déjà passées (nowMs)', () => {
    const cache = {};
    const from = new Date(2026, 5, 21);
    const all = upcomingPlanetOccurrences('Vénus', from, 3, LAT, LNG, cache, from.getTime() - 1);
    const laterHalf = upcomingPlanetOccurrences('Vénus', from, 3, LAT, LNG, cache, all[3].start.getTime());
    expect(laterHalf.length).toBeLessThan(all.length);
    expect(laterHalf.every((o) => o.start.getTime() > all[3].start.getTime())).toBe(true);
  });

  it('ne renvoie que la planète demandée', () => {
    const cache = {};
    const from = new Date(2026, 5, 21);
    const occurrences = upcomingPlanetOccurrences('Mars', from, 4, LAT, LNG, cache, from.getTime() - 1);
    expect(occurrences.every((o) => o.planet === 'Mars')).toBe(true);
  });
});


describe('cohérence page / notification', () => {
  const hour = { planet: 'Mercure', start: new Date('2026-09-24T17:48:00Z'), end: new Date('2026-09-24T18:48:00Z') };
  it('à 18:02:50, Mercure est favorable jusqu’à 18:18', () => {
    const at = new Date('2026-09-24T18:02:50Z');
    const page = planetaryHourState(hour, at);
    const push = planetaryNotification(hour, at, 'Africa/Dakar');
    expect(push.state).toEqual(page);
    expect(push.title).toBe('☿ Mercure');
    expect(push.body).toBe('Favorable (1re moitié) — jusqu’à 18:18');
    expect(page.validUntil.toISOString()).toBe('2026-09-24T18:18:00.000Z');
  });
  it('change de statut et de clé de déduplication exactement à la moitié', () => {
    const first = planetaryHourState(hour, new Date('2026-09-24T18:17:59.999Z'));
    const second = planetaryHourState(hour, new Date('2026-09-24T18:18:00Z'));
    expect(first.nature.cls).toBe('nat-fav');
    expect(second.nature.cls).toBe('nat-def');
    expect(second.stateKey).not.toBe(first.stateKey);
    expect(second.validUntil).toEqual(hour.end);
  });
  it('divise une heure inégale en deux, sans supposer 30 minutes', () => {
    const short = { ...hour, end: new Date('2026-09-24T18:38:00Z') };
    expect(planetaryHourState(short, short.start).validUntil.toISOString()).toBe('2026-09-24T18:13:00.000Z');
  });
  it('ne supprime pas une nouvelle occurrence de la même planète', () => {
    const next = { ...hour, start: new Date(+hour.start + 86400000), end: new Date(+hour.end + 86400000) };
    expect(planetaryHourState(hour, hour.start).stateKey).not.toBe(planetaryHourState(next, next.start).stateKey);
  });
  it('calcule un coucher vers 19h UTC à Dakar, pas 16h45 (longitude ouest)', () => {
    const solar = sunTimesFor(new Date(2026, 8, 24), 14.7, -17.4);
    expect(solar.sunrise.getUTCHours()).toBe(6);
    expect(solar.sunset.getUTCHours()).toBe(19);
    // À latitude égale, un méridien ouest passe après un méridien est.
    const east = sunTimesFor(new Date(2026, 8, 24), 14.7, 17.4);
    expect(+solar.sunrise - +east.sunrise).toBeGreaterThan(2 * 3600000);
  });
  it('respecte le jour civil de Tokyo lorsque le serveur est encore à la veille', () => {
    const now = new Date('2026-09-24T21:30:00Z');
    const pday = computePday(now, 35.68, 139.69, {}, 'Asia/Tokyo');
    expect(pday.dayOfWeek).toBe(5);
    expect(+pday.sunrise).toBeLessThan(+now);
    expect(+pday.nextSunrise).toBeGreaterThan(+now);
  });
});
