...
describe('splitFr', () => {
  it('sépare transcription et description sur le tiret cadratin', () => {
    expect(splitFr('Al-Malik — Le Roi souverain')).toEqual({
      translit: 'Al-Malik',
      desc: 'Le Roi souverain',
    });
  });

  it("gère une description contenant elle-même un tiret cadratin (ne coupe qu'à la première occurrence)", () => {
    expect(splitFr("Al-Bâqî — L'Éternellement Subsistant")).toEqual({
      translit: 'Al-Bâqî',
      desc: "L'Éternellement Subsistant",
    });
  });

  it('sans tiret cadratin : tout dans translit, desc vide', () => {
    expect(splitFr('Allah')).toEqual({ translit: 'Allah', desc: '' });
  });

  it('chaîne vide/nulle : ne plante pas', () => {
    expect(splitFr('')).toEqual({ translit: '', desc: '' });
    expect(splitFr(undefined)).toEqual({ translit: '', desc: '' });
  });
});
