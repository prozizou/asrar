...
// FNV-1a 32 bits : empreinte STABLE et NON réversible, pas un usage
// cryptographique. Sert à identifier une boutique par email sans jamais
// exposer l'adresse réelle au client (server/sources.js retire "email" des
// réponses /api/list-content et /api/get-content — voir emailVendorKey ci-dessous).
function fnv1a(str) {
  let h = 0x811c9dc5;
...
// Identifiant STABLE d'un vendeur à partir de son email, sans jamais renvoyer
// l'email lui-même. Calculé côté serveur (pages/api/list-content.js, à partir
// du document Firestore brut) puis transmis au client dans le champ
// `vendorKey` de chaque produit.
export function emailVendorKey(email) {
  return 'v_' + fnv1a(String(email).trim().toLowerCase());
}

// Identifiant STABLE d'un vendeur : `vendorKey` déjà calculé côté serveur si
// présent, sinon l'email (l'uid peut changer d'un produit à l'autre — compte
// recréé, cf. api/shop.js action "me" — et dupliquerait la même boutique).
export function vendorKey(p) {
  if (p.vendorKey) return p.vendorKey;
  if (p.email) return emailVendorKey(p.email);
  return p.uid || p.vendeurId || 'inconnu';
}
