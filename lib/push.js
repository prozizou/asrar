...
export async function subscribeToPush(position = null) {
  const sub = await ensureBrowserSubscription();
  const { lat, lng } = position || await getPosition();
  await apiPost('push-subscribe', { action: 'subscribe', subscription: sub.toJSON(), lat, lng, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone });
  return sub;
}

// Actualise un abonnement existant avec la position exacte retenue par la page.
// Ne demande aucune permission et ne crée aucun nouvel abonnement.
export async function syncPlanetPushPosition(lat, lng) {
  if (!pushSupported() || lat == null || lng == null) return;
  const reg = await navigator.serviceWorker.ready;
  const sub = await reg.pushManager.getSubscription();
  if (!sub) return;
  await apiPost('push-subscribe', { action: 'subscribe', subscription: sub.toJSON(),
    lat, lng, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone });
}
