...
// lecture Firestore directe, chargée à la demande (Phase 3 de la migration
// RTDB → Firestore, voir docs/FIRESTORE_SCHEMA.md et le commentaire de
// getFirestoreLazy dans lib/firebase.js) → fallback local.
...
export async function loadNames() {
  ...
  try {
    ...
    const snap = await getDocs(collection(firestore, 'asma_ul_husna'));
    const names = [];
    snap.forEach((doc) => {
      const v = doc.data();
      if (validateName(v)) names.push(normalizeName(doc.id, v));
    });
    names.sort(bySort);
    if (names.length) {
      try {
