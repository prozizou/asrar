...
// `sessionAt` (epoch ms, optionnel) : horaire de la prochaine session — sert
// de base au rappel push envoyé aux membres approuvés (lib/reminders.js).
export function createGroup({ name, presetId, arabic, target, private: isPrivate, sessionAt }) {
  return apiPost('zikr', { action: 'create', name, presetId, arabic, target, private: isPrivate, sessionAt });
}
// Créateur only — voir pages/api/zikr.js handleUpdate (la formule récitée se
// verrouille dès que le groupe a commencé à réciter).
export function updateGroup(groupId, { name, presetId, arabic, target, private: isPrivate, sessionAt }) {
  return apiPost('zikr', { action: 'update', groupId, name, presetId, arabic, target, private: isPrivate, sessionAt });
}
export function getGroup(groupId) {
  return apiPost('zikr', { action: 'get', groupId });
}
// `phone` (format international, ex. "+221770000000") — OBLIGATOIRE, voir
// pages/api/zikr.js handleJoin : sert au créateur pour contacter chaque
// participant par WhatsApp (jamais visible des autres membres, cf. handleGet).
export function joinGroup(groupId, phone) {
  return apiPost('zikr', { action: 'join', groupId, phone });
}
...
// Avertit EN UNE FOIS tous les comptes n'ayant récité aucun grain (créateur
// only) — avertissement privé + notification push best-effort si abonné.
// Renvoie `notified` (nombre de comptes visés, 0 si aucun compte inactif).
export function notifyInactiveMembers(groupId) {
  return apiPost('zikr', { action: 'notifyInactive', groupId });
}
...
// Partage (ou retire du partage) SON PROPRE vœu au groupe entier — opt-in,
// un vœu reste privé (créateur + auteur) tant qu'il n'est pas partagé. Voir
// pages/api/zikr.js handleShareWish.
export function shareWish(groupId, shared) {
  return apiPost('zikr', { action: 'shareWish', groupId, shared });
}
// Dit « Amine » (ou le retire, toggle) sur un vœu PARTAGÉ par un autre
// membre — refusé côté serveur sur un vœu resté privé.
export function amineWish(groupId, wishUid) {
  return apiPost('zikr', { action: 'amineWish', groupId, wishUid });
}
// Discussion de groupe façon WhatsApp — réservée aux membres. `media`
// (optionnel) : { type: 'image'|'audio', url, duration? } — déjà hébergé sur
// Cloudinary (voir lib/cloudinary.js uploadZikrChatMedia) AVANT cet appel,
// qui n'enregistre que le pointeur (comme les images produit du marché).
export function sendMessage(groupId, text, media) {
  return apiPost('zikr', {
    action: 'sendMessage',
    groupId,
    text,
    ...
    mediaDuration: media && media.duration,
  });
}
