...
describe('normalizePhone', () => {
  it('accepte un numéro international déjà propre', () => {
    expect(normalizePhone('+221770000000')).toBe('+221770000000');
  ...
  it('accepte le préfixe "00" comme équivalent de "+"', () => {
    expect(normalizePhone('0033612345678')).toBe('+33612345678');
  });

  it('rejette un numéro sans indicatif international ("+"/"00")', () => {
    expect(normalizePhone('770000000')).toBeNull();
    expect(normalizePhone('0612345678')).toBeNull();
  });

  it('rejette trop court, trop long, vide ou non numérique', () => {
    expect(normalizePhone('+1234567')).toBeNull(); // 7 chiffres < min 8
    expect(normalizePhone('+1234567890123456')).toBeNull(); // 16 chiffres > max 15
    expect(normalizePhone('')).toBeNull();
    expect(normalizePhone(undefined)).toBeNull();
    expect(normalizePhone('+33abc12345')).toBeNull();
  });
});

describe('remainingOf', () => {
  it('calcule le reste (target - total)', () => {
    expect(remainingOf(1000, 16641)).toBe(15641);
    expect(remainingOf(0, 100)).toBe(100);
  });

  it('ne descend jamais sous zéro (objectif dépassé ou atteint)', () => {
    expect(remainingOf(100, 100)).toBe(0);
    expect(remainingOf(150, 100)).toBe(0);
  });

  it('traite target/total absents ou invalides comme 0', () => {
    expect(remainingOf(undefined, undefined)).toBe(0);
    expect(remainingOf(NaN, 100)).toBe(100);
  });
});
