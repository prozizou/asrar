...
// ─────────────────────────────────────────────────────────────────────────
// Signature mystique — relie les deux faces (Mashreqi/Maghrébi) entre elles
// plutôt que de les traiter isolément (revue design : « Analyse mystique
// complète »). Fonctions pures, testées indépendamment de l'UI.
// ─────────────────────────────────────────────────────────────────────────
...
// PPCM (plus petit commun multiple) — 0 si l'une des deux valeurs est nulle
// (pas de multiple commun défini dans ce cas).
...
// Décomposition en facteurs premiers : n → [[p, exposant], …], ex.
// 200 → [[2, 3], [5, 2]] (2³ × 5²). Tableau vide pour n < 2.
...
// Notation lisible de la décomposition, ex. "2³ × 5²" ou "2 × 5 × 17"
// (exposant 1 non affiché). "—" pour n < 2 (rien à décomposer).
export function formatFactorization(n) {
  const factors = primeFactorization(n);
  if (!factors.length) return n > 1 ? String(n) : '—';
  return factors.map(([p, e]) => (e > 1 ? `${p}${toSuperscript(e)}` : `${p}`)).join(' × ');
}

// Signature mystique complète d'une paire Mashreqi/Maghrébi : réduction de
// chaque face, union et polarité des faces, produit, racine commune (PGCD)
// et cycle commun (PPCM) — avec leur propre réduction —, union et écart des
// deux réductions, et structure en facteurs premiers des deux faces. Renvoie
// `null` si les deux faces sont nulles (rien à analyser).
export function computeMysticSignature(mash, magh) {
  mash = Math.trunc(Number(mash)) || 0;
  magh = Math.trunc(Number(magh)) || 0;
  if (mash <= 0 && magh <= 0) return null;

  const rMash = reduceNumber(mash);
  const rMagh = reduceNumber(magh);
  const union = mash + magh;
  const polarite = Math.abs(mash - magh);
  const racine = gcd(mash, magh);
  const cycle = lcm(mash, magh);
  const unionReduite = rMash + rMagh;

  return {
    mash, magh, rMash, rMagh,
    union, rUnion: reduceNumber(union),
    polarite, rPolarite: reduceNumber(polarite),
    produit: mash * magh,
    racine, rRacine: reduceNumber(racine),
    cycle, rCycle: reduceNumber(cycle),
    unionReduite, rUnionReduite: reduceNumber(unionReduite),
    ecartReductions: Math.abs(rMash - rMagh),
    factoMash: formatFactorization(mash),
    factoMagh: formatFactorization(magh),
  };
}

// Détail lettre par lettre (revue design : « correspondances Abjad lettres
// ↔ nombres ») — [] pour une entrée purement numérique (normalizeArabic()
// ne garde que les lettres arabes, les chiffres sont donc naturellement
// exclus) ou vide.
export function letterBreakdown(input) {
  const normalized = normalizeArabic(input);
  if (!normalized) return [];
  return normalized.split('').map((letter) => ({
    letter,
    mash: getLetterValue(letter, 'mashriqi'),
    magh: getLetterValue(letter, 'maghrebi'),
  }));
}
