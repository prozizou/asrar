...
describe('composePhrasePages', () => {
  ...
  it('touche le bord RÉEL de la boucle à la hauteur du tatweel (± le chevauchement voulu), pas sa demi-largeur maximale (cx±a)', () => {
    ...
    // qu'exactement à cy) — à un léger chevauchement voulu près (voir test
    // dédié : « aucun espace visible même si la mesure est optimiste »).
    ...
    // Chaque connecteur pénètre la boucle de exactement 1,5× SA PROPRE
    // épaisseur au-delà du contour réel (chevauchement volontaire, voir
    // test dédié) — cette épaisseur dépend de ce qui précède CE connecteur
    // (texte ou boucle), pas de la boucle touchée elle-même.
    const touchXs = page.connectors
      .flatMap((c) => [c.x1, c.x2].map((x) => ({ x, overlap: c.width * 1.5 })))
      .filter(({ x, overlap }) => Math.abs(Math.abs(x - loop.cx) - (halfAtTextY - overlap)) < 0.01);
    expect(touchXs).toHaveLength(2); // un par connecteur, du bon côté
  });

  it('chevauche légèrement le TEXTE aussi — jamais d’espace visible de ce côté', () => {
    // Demandé explicitement : « le tatweel [doit] se coller parfaitement à
    // la lettre qui la précède, ne laissant aucun espace ». measure() donne
    // l'avance de police, pas toujours l'encre visible du glyphe — le trait
    // empiète donc DANS le texte plutôt que de s'arrêter pile à son bord
    // théorique.
    const p = composePhrasePages({ phrase: 'بمر', letters: ['م'], innerText: 'يا رزاق', measure });
    const page = p.pages[0];
    const first = page.items.find((it) => it.type === 'text'); // « ب », touché par le premier connecteur sur son bord gauche
    const width = measure(first.value, first.fontSize);
    const itemLeft = first.x - width / 2;
    const overlap = Math.max(2, first.fontSize * 0.09) * 1.5;
    expect(page.connectors[0].x1).toBeCloseTo(itemLeft + overlap, 1);
    expect(page.connectors[0].x1).toBeGreaterThan(itemLeft); // dépasse DEDANS le texte, jamais pile dessus
  });
