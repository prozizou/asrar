...
describe('tokenizePhrase', () => {
  ...
  it('une occurrence de ة (via EQUIV) ne joint jamais vers l’avant, même dessinée comme un ه', () => {
    ...
    // ce qui doit se passer entre un ة et le mot suivant.
    const after = tokens[tokens.indexOf(loop) + 1];
    expect(after.value).toBe(' يا رب');
  });
...
describe('composePhrasePages', () => {
  ...
  it('ne trace le tatweel qu’aux jonctions où la lettre précédente joint réellement vers l’avant', () => {
    // « برق الليل » avec ق gonflé : « بر » précède la boucle mais finit par
    // ر, qui ne joint jamais vers l'avant — aucun connecteur à cette
    // jonction. La boucle (ق, dual-joining) joint bien vers le segment
    // suivant (" الليل") : ce second connecteur, lui, doit exister.
    const p = composePhrasePages({ phrase: 'برق الليل', letters: ['ق'], innerText: 'يا رزاق', measure });
    const page = p.pages[0];
    expect(page.connectors).toHaveLength(1);
    const loop = page.items.find((it) => it.type === 'loop');
    expect(loop).toBeDefined();
    // Et son épaisseur est celle de la boucle qui le précède (voir test
    // suivant pour le cas symétrique côté texte).
    expect(page.connectors[0].width).toBe(loop.stroke);
  });
