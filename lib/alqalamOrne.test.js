...
describe('composePiece — mot affiché', () => {
  ...
  it('garde le reste du mot pour une lettre au milieu (رزقك + ق)', () => {
    const p = composePiece({ word: 'رزقك', letter: 'ق', innerText: '', measure });
    expect(p.word.replace(/‍/g, '')).toBe('رزك');
  });

  it('lie les deux moitiés au point de coupe (elles doivent se souder à la boucle, pas s’en détacher)', () => {
    // Signalé : sans ce liant, le س de « بسم » (م gonflé) retombe sur sa
    // forme isolée/finale — la boucle doit au contraire se lire comme un
    // prolongement du trait, donc le س doit garder sa forme MÉDIANE.
    const p = stripLetterOccurrences('رزقك', 'ق');
    expect(p).toBe('رز‍ك');
  });

  it('ne place aucun liant en tout début ou toute fin de mot', () => {
    expect(stripLetterOccurrences('الله', 'ه')).toBe('الل'); // ه final : rien après
    expect(stripLetterOccurrences('ممد', 'م')).toBe('د'); // occurrences en tête : rien avant
  });
});
...
describe('composePhrasePiece', () => {
  ...
  it('respecte les bornes de page (largeur ET hauteur)', () => {
    const p = composePhrasePiece({ phrase: BASMALA, letters: ['م', 'ه'], innerText: VOEU, measure });
    expect(p.H).toBeLessThanOrEqual(PIECE.H_MAX);
