...
it('mesure les formes liées avec le même sens RTL que le SVG', () => {
  const ctx = { direction: 'ltr', font: '', measureText: vi.fn(() => ({ width: 42 })) };
  vi.stubGlobal('document', { createElement: () => ({ getContext: () => ctx }) });
  try {
    expect(makeMeasurer('serif')('بس\u200d', 40)).toBe(42);
    expect(ctx.direction).toBe('rtl');
    expect(ctx.measureText).toHaveBeenCalledWith('بس\u200d');
  } finally {
    vi.unstubAllGlobals();
  }
});
...
describe('bowlPath', () => {
  ...
  it('trace un DÔME BIEN FERMÉ (courbe en haut, trait droit en bas) quand arcSpan < 2π — demandé pour ص ض ط ظ', () => {
    const d = bowlPath(100, 80, 3, 240, ARC_SPAN);
    expect(d.startsWith('M')).toBe(true);
    expect(d.endsWith('Z')).toBe(true); // bien fermé, comme un rond plein — jamais un tracé ouvert
    // Premier et dernier point tracés à y=0 (la base du dôme) : c'est le
    // trait droit que 'Z' referme entre eux qui forme le bas plat.
    const coords = d.replace(/^M/, '').replace(/Z$/, '').split('L');
    const [, firstY] = coords[0].split(',').map(Number);
    const [, lastY] = coords[coords.length - 1].split(',').map(Number);
    expect(firstY).toBeCloseTo(0, 1);
    expect(lastY).toBeCloseTo(0, 1);
  });

  it('reste dans la boîte englobante même en dôme', () => {
    const a = 120;
    const b = 90;
...
describe('tokenizePhrase', () => {
  it('conserve la liaison avant une boucle après les voyelles et la chadda', () => {
    for (const prefix of ['بِسْ', 'لَّ', 'عَلَيْ']) {
      const [text] = tokenizePhrase(prefix + 'م', ['م']);
      expect(text.value).toBe(prefix + '\u200d');
      expect(text.joinsForward).toBe(true);
    }
    const [text] = tokenizePhrase('رَح', ['ح']);
    expect(text.value).toBe('رَ');
...
describe('paginateRows', () => {
  ...
  it('répartit sur plusieurs pages quand ça déborde', () => {
    ...
    expect(pages[2]).toEqual([6, 7]);
    expect(pages[3]).toEqual([8, 9]);
  });
...
describe('composePhrasePages', () => {
  ...
  it('répartit les lignes de la dernière page vers le bas sans perdre la marge ni le numéro', () => {
    const p = composePhrasePages({
      phrase: Array(4).fill(BASMALA).join(' '), letters: ['م', 'ه'], innerText: 'رزق', measure,
    });
    expect(p.pageCount).toBe(2);
    const last = p.pages[1];
    const loops = last.items.filter((it) => it.type === 'loop');
    const bottom = Math.max(...loops.map((it) => it.cy + shapeFor('م', p.radius).b));
    expect(bottom).toBeGreaterThan(last.H * .8);
    expect(bottom).toBeLessThan(last.H - PHRASE_PIECE.MARGIN - PHRASE_PIECE.FOOTER_HEIGHT);
  });
  ...
  it('prolonge les raccords dans le texte des deux côtés de la boucle', () => {
    const { pages: [page] } = composePhrasePages({ phrase: 'بمر', letters: ['م'], innerText: 'يا رزاق', measure });
    const [before, after] = page.items.filter(item => item.type === 'text');
    expect(page.connectors[0].x1).toBeGreaterThan(before.x - measure(before.value, before.fontSize) / 2);
    expect(page.connectors[1].x2).toBeLessThan(after.x + measure(after.value, after.fontSize) / 2);
  });

  it('utilise la hauteur et l’épaisseur de l’encre et s’arrête au contour des boucles', () => {
    const inkMeasure = Object.assign((text, size) => measure(text, size), {
      connection: (text, size, side) => ({
        x: side === 'left' ? 2 : measure(text, size) - 2,
        y: -3,
        width: 2.5,
      }),
    });
    try {
      const { pages: [page], radius } = composePhrasePages({ phrase: 'بمر', letters: ['م'], innerText: 'يا رزاق', measure: inkMeasure });
      const [before, after] = page.items.filter(item => item.type === 'text');
      const loop = page.items.find(item => item.type === 'loop');
      const shape = shapeFor('م', radius);
      const half = halfWidthAt(before.y - 3 - loop.cy, shape.a, shape.b, shape.n);
      expect(page.connectors[0]).toMatchObject({ y: before.y - 3, width: 2.5 });
      expect(page.connectors[1]).toMatchObject({ y: after.y - 3, width: 2.5 });
      expect(page.connectors[0].x2).toBeCloseTo(loop.cx + half);
      expect(page.connectors[1].x1).toBeCloseTo(loop.cx - half);
    } finally {
      delete inkMeasure.connection;
    }
  });
  ...
  it('touche le bord RÉEL de la boucle à la hauteur du tatweel, pas sa demi-largeur maximale (cx±a)', () => {
    ...
    // qu'exactement à cy). Pas de chevauchement volontaire (essayé, puis
    // retiré — ça débordait dans les boucles étroites, signalé comme moins
    // esthétique) : le point de contact est exact, sans marge ajoutée.
    ...
    // Le trait passe sous le contour pour éviter une couture blanche.
    const touchXs = page.connectors
      .flatMap((c) => [c.x1, c.x2])
      .filter((x) => Math.abs(Math.abs(x - loop.cx) - halfAtTextY) <= loop.stroke / 2 + 0.01);
    expect(touchXs).toHaveLength(2); // un par connecteur, du bon côté
  });

  it('recentre un dôme (ص ض ط ظ) sur la même base qu’un rond plein voisin — sinon il flotte trop haut', () => {
    // Signalé avec une capture au zoom : un dôme (ARC_SPAN, n'occupe que
    // [-b, 0] dans son propre repère) rendu à la même translation `cy`
    // qu'un rond plein (qui occupe [-b, b], centré sur cy) se retrouve
    // décalé d'un demi-b par rapport à un rond voisin sur la même ligne.
    // loopCy = cy + b/2 pour un dôme corrige ça : son occupation devient
    // [cy-b/2, cy+b/2], centrée sur le même cy que le rond.
    const bowlYRange = (bowl) => {
      const ys = bowl
        .replace(/[MLQZ]/g, ' ')
        .trim()
        .split(/\s+/)
    ...
    const p = composePhrasePages({ phrase: 'صم صم', letters: ['ص', 'م'], innerText: 'رزق', measure });
    const page = p.pages[0];
    const loops = page.items.filter((it) => it.type === 'loop');
    expect(loops.length).toBeGreaterThanOrEqual(2);

    // Le rond (م) n'est jamais recentré : son cy EST la base de ligne.
    const roundLoop = loops.find((l) => bowlYRange(l.bowl).max > 0.01);
    // Le dôme (ص) n'occupe que [-b, 0] dans son propre repère : max ≈ 0.
    const domeLoop = loops.find((l) => bowlYRange(l.bowl).max <= 0.01);
    expect(roundLoop).toBeDefined();
    expect(domeLoop).toBeDefined();

    const { min, max } = bowlYRange(domeLoop.bowl);
    const domeAbsoluteCenter = domeLoop.cy + (min + max) / 2;
    expect(domeAbsoluteCenter).toBeCloseTo(roundLoop.cy, 1);
  });

  it('le tatweel touche un dôme (ص ض ط ظ) SUR SON TRAIT PLAT — jamais à la hauteur générique du texte, qui tombe en pleine courbe', () => {
    // Signalé avec une capture au zoom (« doit être dans la ligne droite,
    // pas en haut ») : à la hauteur textY générique, un dôme (occupant
    // seulement [-b, 0] dans son repère recentré) est touché en pleine
    // COURBE, pas sur son trait plat — le tatweel grimpait visiblement vers
    // le haut au lieu de courir droit. Un connecteur qui touche un dôme
    // doit donc être à LA hauteur de son trait plat (loop.cy — y local 0),
    // et toucher EXACTEMENT au coin (demi-largeur maximale a), jamais un
    // point en retrait pris sur la courbe.
    const p = composePhrasePages({ phrase: 'بصر', letters: ['ص'], innerText: 'رزق', measure });
    const page = p.pages[0];
    expect(page.connectors).toHaveLength(2); // ب→dôme et dôme→ر
    const loop = page.items.find((it) => it.type === 'loop');
    const shape = shapeFor('ص', p.radius);
    for (const c of page.connectors) {
      expect(c.y).toBeCloseTo(loop.cy, 5);
    }
    const touchesLoop = page.connectors
      .flatMap((c) => [c.x1, c.x2])
      .filter((x) => Math.abs(Math.abs(x - loop.cx) - shape.a) <= loop.stroke / 2 + 0.01);
    expect(touchesLoop).toHaveLength(2); // un par connecteur, du bon côté
  });

  it('ne réserve plus le double de hauteur de ligne pour un dôme (ص ض ط ظ) — sinon la page ne se remplit qu’à moitié', () => {
    // Signalé avec une capture d'un PDF aux pages moitié blanches : un dôme
    // recentré n'occupe que [cy-b/2, cy+b/2] (voir le test de recentrage
    // ci-dessus) — la MOITIÉ de la place d'un rond plein, qui occupe
    // [cy-b, cy+b]. Compter `b` plein (comme pour un rond) pour calculer la
    // hauteur de ligne réservait deux fois plus de place que nécessaire.
    const dua = Array(30).fill('بصر').join(' ');
    const p = composePhrasePages({ phrase: dua, letters: ['ص'], innerText: 'رزق', measure });
    expect(p.pageCount).toBeGreaterThan(1);
    // Sur une page pleine (la première, forcément non partielle puisqu'il y
    // en a d'autres après), les boucles doivent couvrir la quasi-totalité
    // de la hauteur utile de la page — jamais seulement la moitié.
    const page = p.pages[0];
    const loops = page.items.filter((it) => it.type === 'loop');
    const shape = shapeFor('ص', p.radius);
    const spanTop = Math.min(...loops.map((l) => l.cy)) - shape.b / 2;
    const spanBottom = Math.max(...loops.map((l) => l.cy)) + shape.b / 2;
    const maxHeight = PHRASE_PIECE.H_MAX - 2 * PHRASE_PIECE.MARGIN;
    expect(spanBottom - spanTop).toBeGreaterThan(maxHeight * 0.7);
  });
  ...
  it('signale un débordement du VŒU si trop long, sans affecter la mise en page (les boucles restent)', () => {
    ...
    // Les boucles sont bien là, mais au moins une ne peut contenir le vœu.
    const loops = allItems(p).filter((it) => it.type === 'loop');
    expect(loops.length).toBeGreaterThan(0);
    expect(loops.some((l) => l.inner === null)).toBe(true);
  });
  ...
  it('signale un débordement de PAGES quand même la pagination ne suffit plus (MAX_PAGES dépassé)', () => {
    ...
    const phrase = Array(3000).fill('محمد').join(' ');
...
describe('Ornement A4 paysage prêt à exporter', () => {
  it('exporte une composition de plus de 40 pages si toutes les boucles sont complètes', () => {
    const p = composePhrasePages({ phrase: Array(300).fill('محمد').join(' '), letters: ['م'], innerText: 'يا رزاق', measure });
    expect(p.pageCount).toBeGreaterThan(40);
    expect(p.overflow).toBe(false);
    expect(p.pages.every(page => page.items.filter(it => it.type === 'loop').every(it => it.inner))).toBe(true);
  });

  it('remplit les boucles avec un long vœu et garde les raccords sous les lettres', () => {
    const p = composePhrasePages({
      phrase: 'اللهم اجعلنا من المقربين في جنات النعيم',
      letters: ['م', 'ه'],
      innerText: Array(100).fill('اللهم ارزقني رزقا واسعا حلالا طيبا').join(' '),
      measure,
    });
    expect(p.overflow).toBe(false);
    expect(p.pages[0].items.filter(it => it.type === 'loop').every(it => it.inner?.lines.length)).toBe(true);
    expect(p.pages.flatMap(page => page.items).filter(it => it.type === 'text').every(it => it.wordSpacing === 0)).toBe(true);
    expect(p.pages.some(page => page.connectors.length > 0)).toBe(true);
  });

  it('réserve 5 mm et un pied de page, y compris autour des contours', () => {
    expect(PHRASE_PIECE.MARGIN * 297 / PHRASE_PIECE.W).toBeCloseTo(5, 10);
    const p = composePhrasePages({ phrase: Array(25).fill('بسم الله الرحمن الرحيم').join(' '), letters: ['م', 'ه'], innerText: 'يا رزاق', measure });
    expect(p.overflow).toBe(false);
    for (const page of p.pages) {
      for (const item of page.items.filter(i => i.type === 'loop')) {
        const points = item.bowl.match(/-?\d+(?:\.\d+)?,-?\d+(?:\.\d+)?/g).map(p => p.split(',').map(Number));
        for (const [x, y] of points) {
          expect(item.cx + x - item.stroke / 2).toBeGreaterThanOrEqual(PHRASE_PIECE.MARGIN - .01);
          expect(item.cx + x + item.stroke / 2).toBeLessThanOrEqual(page.W - PHRASE_PIECE.MARGIN + .01);
          expect(item.cy + y - item.stroke / 2).toBeGreaterThanOrEqual(PHRASE_PIECE.MARGIN - .01);
          expect(item.cy + y + item.stroke / 2).toBeLessThanOrEqual(page.H - PHRASE_PIECE.MARGIN - PHRASE_PIECE.FOOTER_HEIGHT + .01);
        }
      }
    }
  });

  it('justifie par les espaces sans changer la taille des lettres et garde la dernière ligne naturelle', () => {
    const p = composePhrasePages({ phrase: Array(50).fill('يا رزاق يا كريم').join(' '), letters: [], innerText: '', measure });
    const text = p.pages.flatMap(page => page.items).filter(i => i.type === 'text');
    expect(text.some(i => i.wordSpacing > 0)).toBe(true);
    expect(text.at(-1).wordSpacing).toBe(0);
    expect(new Set(text.map(i => i.fontSize)).size).toBe(1);
    for (const item of text.filter(i => i.wordSpacing > 0)) {
      const width = measure(item.value, item.fontSize) + item.wordSpacing * (item.value.match(/ /g) || []).length;
      expect(item.x - width / 2).toBeGreaterThanOrEqual(PHRASE_PIECE.MARGIN + PHRASE_PIECE.INK_INSET - .01);
      expect(item.wordSpacing).toBeLessThanOrEqual(p.radius * .22 + .01);
    }
  });
});
