...
describe('ENDINGS — liste corrigée', () => {
  ...
  it('contient les 4 finitions du premier ajout avec le bon poids', () => {
  ...
  it('contient les 10 finitions du second ajout avec le bon poids', () => {
    expect(ENDINGS).toEqual(
      expect.arrayContaining(['عيوش', 'ميش', 'ءيل', 'ئيل', 'يئيل', 'دوش', 'لوش', 'يوش', 'هيش', 'كطيش'])
    );
    expect(calculateWeight('عيوش')).toBe(1086); // ع70 + ي10 + و6 + ش1000
    expect(calculateWeight('ميش')).toBe(1050); // م40 + ي10 + ش1000
    expect(calculateWeight('ءيل')).toBe(41); // ء1 + ي10 + ل30 — ne finit PAS par ش
    expect(calculateWeight('ئيل')).toBe(50); // ئ10 + ي10 + ل30 — ne finit PAS par ش
    expect(calculateWeight('يئيل')).toBe(60); // ي10 + ئ10 + ي10 + ل30 — ne finit PAS par ش
    expect(calculateWeight('دوش')).toBe(1010); // د4 + و6 + ش1000
    expect(calculateWeight('لوش')).toBe(1036); // ل30 + و6 + ش1000
    expect(calculateWeight('يوش')).toBe(1016); // ي10 + و6 + ش1000
    expect(calculateWeight('هيش')).toBe(1015); // ه5 + ي10 + ش1000
    expect(calculateWeight('كطيش')).toBe(1039); // ك20 + ط9 + ي10 + ش1000
  });

  it('a 19 finitions au total, toutes distinctes', () => {
    expect(ENDINGS.length).toBe(19);
    expect(new Set(ENDINGS).size).toBe(19);
  });
});
...
describe('generateThalsams — mode automatique (longueur + finition)', () => {
  it('teste plusieurs finitions et longueurs, tous les résultats respectent la cible', () => {
    // La plupart des finitions pèsent au moins 1000 (ش) ; 3 d'entre elles
    // (ءيل/ئيل/يئيل, en ل) pèsent beaucoup moins (41/50/60) — peu importe
    // laquelle est utilisée au final, seul compte le résultat : poids exact
    // et longueur dans la fourchette demandée.
    const out = generateThalsams({
      targetWeight: 1090,
  ...
  it('les finitions en ل (ءيل/ئيل/يئيل, hors motif ش) fonctionnent aussi', () => {
    // ءيل (ء1+ي10+ل30=41, 3 lettres) + racine ا(1) = 42, 4 lettres.
    const out = generateThalsams({
      targetWeight: 42,
      length: { mode: 'exact', exact: 4 },
      ending: { mode: 'specific', selected: 'ءيل' },
    });
    expect(out.totalResults).toBeGreaterThan(0);
    for (const r of out.results) {
      expect(r.totalWeight).toBe(42);
      expect(r.ending).toBe('ءيل');
      expect(r.thalsam.endsWith('ءيل')).toBe(true);
      expect(validateResult(r, 42)).toBe(true);
    }
  });
});
