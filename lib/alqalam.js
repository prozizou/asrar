...
/** @param {{onProgress?: (pct: number, text?: string) => void, [k: string]: any}} opts */
...
// Exportés : l'écriture ornée (lib/alqalamOrne.js) imprime par la MÊME voie.
// Deux mécanismes d'impression concurrents se neutraliseraient — la règle
// `body > *:not(#racine)` de l'un masquerait la racine de l'autre, avec une
// spécificité supérieure à celle qui l'affiche.
export const PDF_PRINT_ROOT_ID = 'alqalam-print-root';
// Exporté : l'écriture ornée (lib/alqalamOrne.js) désactive temporairement
// CETTE feuille pendant SON impression (voir printPieces) — deux @page
// concurrents (celui-ci en portrait implicite, le sien en paysage) dans
// deux <style> distincts ne se départagent pas de façon fiable sur tous
// les moteurs, notamment mobiles.
export const PDF_PRINT_STYLE_ID = 'alqalam-print-style';
...
// Calibri est une police Microsoft sous licence : on ne peut pas embarquer
// ses fichiers dans l'app (ça les redistribuerait à chaque visiteur, ce qui
// viole sa licence — c'est pour ça que Carlito, libre, est chargée via Google
// Fonts juste après). En revanche, la lister EN PREMIER ne coûte rien : sur
// une machine où Calibri est déjà installée (Windows/Office — très
// probablement le cas ici), le navigateur utilise directement cette copie
// locale sans rien télécharger ; sur les autres, il retombe sur Carlito.
//
// Pour le texte arabe (l'essentiel du document, non couvert par Calibri ni
// Carlito), on tombe sur la suite de la pile. « sans-serif » seul laissait le
// navigateur choisir une police arabe au hasard, souvent sans réelle graisse
// Bold dessinée (juste un faux-gras approximatif, ou rien de visible) — d'où
// le rendu perçu comme non-gras. On liste explicitement des polices arabes
// très répandues qui possèdent, elles, une vraie graisse Bold (Segoe UI/
// Tahoma sous Windows, Noto Naskh/Sans Arabic ailleurs).
const pdfTextStyle = (fontSizePt) => `
  font-family: Calibri, Carlito, 'Segoe UI', Tahoma, Arial, 'Noto Naskh Arabic', 'Noto Sans Arabic', sans-serif;
  font-weight: 700;
...
export function ensurePrintStylesInjected() {
...
function withTimeout(promise, ms = DATA_TIMEOUT_MS) {
  ...
  return Promise.race([
    promise,
    new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Délai dépassé')), ms); }),
  ]).finally(() => clearTimeout(timer));
}

function cachedSourates() {
  try {
    ...
    if (cached && typeof cached === 'object') {
      ...
      for (const [key, data] of Object.entries(cached)) {
        if (data && typeof data.sourate === 'string' && typeof data.contenu === 'string') {
          content[key] = data.contenu;
          list.push({ key, name: data.sourate });
        }
      }
      if (list.length) return { list, content, offline: true };
    }
  } catch {}
  return null;
}

export async function loadSourates(onCached) {
  const cached = cachedSourates();
  if (cached) onCached?.(cached);
  try {
    const snap = await withTimeout(get(ref(db, 'sourate')));
    if (snap.exists()) {
      const list = [];
      const content = {};
      snap.forEach((child) => {
  ...
  return cached || { list: [], content: {}, offline: false };
}

export async function loadVersets() {
  try {
    const snap = await withTimeout(get(ref(db, 'versetRef')));
    if (snap.exists()) {
      const arr = [];
      snap.forEach((child) => {
        const data = child.val();
...
export function loadQuranUthmani() {
  ...
  quranUthmaniPromise = (async () => {
    try {
      const res = await fetch(QURAN_UTHMANI_URL, { signal: AbortSignal.timeout(DATA_TIMEOUT_MS) });
      if (!res.ok) throw new Error('API Coran indisponible');
...
export function matchUthmaniSourate(surahs, sourateName) {
  ...
  // Un marqueur '﴾' APRÈS chaque verset (dernier compris), comme dans le
  // contenu « sans voyelles » existant (buildIntercalatedText s'appuie
  // dessus pour insérer la phrase entre — et après — chaque verset). Un
  // simple .join(marqueur) ne place le marqueur qu'ENTRE les versets (N-1
  // marqueurs pour N versets) : la dernière intercalation manquait alors à
  // l'appel (ex. sourate 94, 8 versets → seulement 7 intercalations) — d'où
  // le marqueur final ajouté ci-dessous. ﻿ : BOM présent en tête de
  // certains versets renvoyés par l'API — invisible mais à retirer proprement.
