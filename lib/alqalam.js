...
// ── Génération PDF ───────────────────────────────────────────────────────────
// html2pdf.js (déjà une dépendance — voir lib/pdf.js pour les secrets) rend un
// vrai DOM via le moteur de rendu du navigateur : c'est la seule façon fiable
// d'obtenir une mise en forme RTL + justifiée correcte pour de l'arabe —
// réimplémenter la segmentation/le rendu du texte arabe à la main dans une
// librairie PDF bas niveau (jsPDF seul) serait beaucoup plus fragile. Le
// pied de page personnalisé (numéro dans une forme de rouleau) est ajouté
// ENSUITE en parcourant directement l'instance jsPDF : html2pdf seul n'a pas
// de notion d'en-tête/pied de page répété sur chaque page.
//
...
const PDF_PAGE_W_CM = 21; // A4 portrait
const PDF_PAGE_H_CM = 29.7;
const PDF_MARGIN_CM = 0.3; // haut/bas/gauche/droite
const PDF_FOOTER_SHAPE_H_CM = 0.8; // hauteur de la forme « rouleau »
const PDF_FOOTER_GAP_CM = 0.25; // espace entre le texte et le rouleau
const PDF_FOOTER_RESERVE_CM = PDF_FOOTER_SHAPE_H_CM + PDF_FOOTER_GAP_CM;
const PDF_CONTENT_W_CM = PDF_PAGE_W_CM - 2 * PDF_MARGIN_CM;
const PDF_CONTENT_H_CM = PDF_PAGE_H_CM - 2 * PDF_MARGIN_CM - PDF_FOOTER_RESERVE_CM;
...
// Découpe le texte en petits groupes de mots, chacun dans un span
// inline-block marqué break-inside/page-break-inside:avoid — html2pdf
// tranche un CANEVAS (une image du rendu), donc sans repère de « ne pas
// couper ici », une ligne à cheval sur la limite d'une page se retrouvait
// littéralement coupée en deux au milieu des lettres (retour utilisateur).
// Un groupe entier n'est jamais scindé : le passage à la page suivante se
// fait toujours ENTRE deux groupes, jamais au travers. html2pdf respecte
// nativement ces propriétés CSS sur tout élément (mode par défaut : 'css',
// voir pagebreaks.js dans html2pdf.js — pas besoin de config supplémentaire).
const PDF_CHUNK_WORDS = 8;
function buildChunkedTextHtml(text) {
  const words = text.split(/\s+/).filter(Boolean);
  const html = [];
  for (let i = 0; i < words.length; i += PDF_CHUNK_WORDS) {
    const chunk = words.slice(i, i + PDF_CHUNK_WORDS).join(' ');
    html.push(`<span style="display:inline-block;break-inside:avoid;page-break-inside:avoid;">${escapeHtml(chunk)}</span>`);
  }
  return html.join(' ');
}

// Mesure la hauteur réelle du texte à chaque taille candidate (DOM caché, sans
// passer par html2canvas — bien plus rapide) et choisit celle qui remplit le
// mieux la dernière page, pour éviter un grand blanc final. Mesuré sur le
// MÊME balisage (groupes de mots) que le rendu final, pour une hauteur fidèle.
function pickBestFontSize(text) {
  const chunkedHtml = buildChunkedTextHtml(text);
  ...
  for (const size of PDF_FONT_CANDIDATES) {
    ...
    el.style.cssText = 'position:fixed;left:-99999px;top:0;visibility:hidden;' + pdfTextStyle(size);
    el.innerHTML = chunkedHtml;
...
// Pied de page répété sur chaque page : numéro centré dans une forme de
// rouleau horizontal, collée au bord bas (distance 0 cm, comme demandé) —
// dessinée directement via l'API vectorielle de jsPDF (html2pdf n'a pas de
// notion de pied de page répété automatiquement). Retour utilisateur : le
// remplissage doré plein n'était pas voulu — juste le contour du rouleau
// (transparent) et le numéro en noir sur blanc.
function drawScrollFooters(pdf) {
  const total = pdf.internal.getNumberOfPages();
  const scrollW = 3.6;
  const scrollH = PDF_FOOTER_SHAPE_H_CM;
  const cx = PDF_PAGE_W_CM / 2;
  const cy = PDF_PAGE_H_CM - scrollH / 2; // bord bas du rouleau = bord bas de la page (0 cm)

  for (let i = 1; i <= total; i++) {
    pdf.setPage(i);
    pdf.setDrawColor(120, 120, 120);
    pdf.setLineWidth(0.02);
    // 'S' = contour seul, aucun remplissage (transparent).
    pdf.roundedRect(cx - scrollW / 2, cy - scrollH / 2, scrollW, scrollH, scrollH / 2, scrollH / 2, 'S');
    pdf.circle(cx - scrollW / 2, cy, scrollH / 2, 'S');
    pdf.circle(cx + scrollW / 2, cy, scrollH / 2, 'S');

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(11);
    pdf.setTextColor(0, 0, 0);
    pdf.text(String(i), cx, cy, { align: 'center', baseline: 'middle' });
  }
}

// Génère et télécharge le .pdf — mêmes réglages (ouverture/fermeture, blocs,
// mode Rasm) que generateDocx, format de sortie différent. onProgress(pct, text).
export async function generatePdf({ useOuv, useFerm, blocks, docName, onProgress = () => {} }) {
  onProgress(5, 'Préparation du PDF…');
  await new Promise((r) => setTimeout(r, 0));
  ...
  onProgress(15, 'Chargement de la police…');
  await ensureCarlitoLoaded();

  onProgress(25, 'Calibration de la mise en page…');
  await new Promise((r) => setTimeout(r, 0));
  const fontSizePt = pickBestFontSize(fullText);

  onProgress(40, 'Génération du PDF…');
  // Élément DÉTACHÉ (jamais inséré dans le document), comme downloadSecretPdf
  // (lib/pdf.js) qui fonctionne déjà de façon fiable avec html2pdf. Un essai
  // précédent l'insérait dans le DOM en position:fixed;left:-99999px (très
  // loin hors écran) : html2canvas n'arrivait pas à le capturer correctement
  // à cette distance — PDF vide, seul le pied de page (dessiné à part, via
  // l'API jsPDF) apparaissait.
  const container = document.createElement('div');
  container.style.cssText = pdfTextStyle(fontSizePt);
  container.innerHTML = buildChunkedTextHtml(fullText);

  const html2pdf = (await import('html2pdf.js')).default;
  const worker = html2pdf()
    .set({
      margin: [PDF_MARGIN_CM, PDF_MARGIN_CM, PDF_MARGIN_CM + PDF_FOOTER_RESERVE_CM, PDF_MARGIN_CM],
      filename: (docName || 'document') + '.pdf',
      // PNG (sans perte) au lieu de JPEG : du texte noir sur blanc n'a pas de
      // dégradés/photo à compresser, et JPEG floute légèrement les contours
      // nets des lettres (artefacts de compression) — sensible au zoom.
      image: { type: 'png' },
      // scale 3 (au lieu de 2) : le rendu reste une image (nécessaire pour la
      // mise en forme arabe RTL/justifiée correcte, gérée par le moteur du
      // navigateur) — donc jamais aussi net qu'un vrai texte vectoriel au
      // zoom, mais une résolution plus élevée réduit nettement le flou.
      html2canvas: { scale: 3, useCORS: true, backgroundColor: '#ffffff' },
      jsPDF: { unit: 'cm', format: 'a4', orientation: 'portrait' },
    })
    .from(container);

  onProgress(70, 'Mise en page multi-page…');
  const pdf = await worker.toPdf().get('pdf');

  onProgress(85, 'Numérotation des pages…');
  drawScrollFooters(pdf);

  onProgress(95, 'Téléchargement…');
