...
function htmlToRuns(html, size) {
  return segmentsToRuns(htmlToColorSegments(html), size);
}
...
// limite aussi le nombre de runs dans le .docx final).
...
export async function generateDocx({ useOuv, useFerm, blocks, docName, fontPx = 28, onProgress = () => {} }) {
  onProgress(10, 'Préparation du Word…');
  await new Promise((r) => setTimeout(r, 0));

  const halfPoints = Math.max(16, Math.round(fontPx * 1.5));
  ...
  const runs = [];
  if (useOuv) pushAll(runs, htmlToRuns(appliquerCouleursManuscrit(formules.ouverture, firstRasm), halfPoints));

  onProgress(45, 'Assemblage du texte…');
  await new Promise((r) => setTimeout(r, 0));

  for (const block of blocks) {
    const coloredBase = appliquerCouleursManuscrit(block.texte, block.isRasmMode) + ' ';
    const n = block.totalMultiplier || 1;
    // Sanitize + parcours DOM sur la SEULE unité (petite), pas sur le texte
    // répété (qui peut peser plusieurs Mo pour n proche de MAX_TOTAL_REPEAT) —
    // c'était la cause du temps d'assemblage très long sur les gros documents.
    const unitSegments = htmlToColorSegments(coloredBase);
    pushAll(runs, segmentsToRuns(repeatSegments(unitSegments, n), halfPoints));
    // Laisse la barre de progression respirer entre deux blocs volumineux.
    await new Promise((r) => setTimeout(r, 0));
  }
  if (useFerm) pushAll(runs, htmlToRuns(appliquerCouleursManuscrit(formules.fermeture, lastRasm), halfPoints));
  ...
  onProgress(80, 'Assemblage du fichier…');
  await new Promise((r) => setTimeout(r, 0));
