...
// mieux la dernière page, pour éviter un grand blanc final.
...
// rouleau horizontal (bandeau arrondi + deux extrémités enroulées), collée
// au bord bas (distance 0 cm, comme demandé) — dessinée directement via
// l'API vectorielle de jsPDF (html2pdf n'a pas de notion de pied de page
...
function drawScrollFooters(pdf) {
  ...
  for (let i = 1; i <= total; i++) {
    pdf.setPage(i);
    pdf.setFillColor(201, 169, 97); // ton parchemin/or — identité mystique de l'app
    pdf.setDrawColor(140, 110, 50);
    pdf.setLineWidth(0.02);
    pdf.roundedRect(cx - scrollW / 2, cy - scrollH / 2, scrollW, scrollH, scrollH / 2, scrollH / 2, 'FD');
    pdf.setFillColor(176, 143, 63);
    pdf.circle(cx - scrollW / 2, cy, scrollH / 2, 'F');
    pdf.circle(cx + scrollW / 2, cy, scrollH / 2, 'F');

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(11);
    pdf.setTextColor(45, 31, 16);
