...
// Mesure la hauteur réelle du texte à chaque taille candidate (DOM caché) et
// choisit celle qui remplit le mieux la dernière page (impression réelle :
// hauteur de page moins les marges @page), pour éviter un grand blanc final.
function pickBestFontSize(text) {
  ...
  for (const size of PDF_FONT_CANDIDATES) {
    ...
    el.style.cssText = 'position:fixed;left:-99999px;top:0;visibility:hidden;width:100%;' + pdfTextStyle(size);
    el.textContent = text;
