...
// du document Word (.docx) et chargement des sourates / versets (Firestore
// via SDK modulaire, chargé à la demande — Phase 3 de la migration RTDB →
// Firestore, voir docs/FIRESTORE_SCHEMA.md et le commentaire de
// getFirestoreLazy dans lib/firebase.js). L'UI vit dans app/alqalam/page.js.
import { getFirestoreLazy } from './firebase';
...
export async function loadSourates() {
  try {
    const [{ collection, getDocs }, firestore] = await Promise.all([import('firebase/firestore'), getFirestoreLazy()]);
    const snap = await getDocs(collection(firestore, 'sourate'));
    if (!snap.empty) {
      const list = [];
      const content = {};
      const cache = {};
      snap.forEach((doc) => {
        const data = doc.data();
        content[doc.id] = data.contenu;
        list.push({ key: doc.id, name: data.sourate });
        cache[doc.id] = data;
      });
      try {
...
export async function loadVersets() {
  try {
    const [{ collection, getDocs }, firestore] = await Promise.all([import('firebase/firestore'), getFirestoreLazy()]);
    const snap = await getDocs(collection(firestore, 'verset_refs'));
    if (!snap.empty) {
      const arr = [];
      snap.forEach((doc) => {
        const data = doc.data();
