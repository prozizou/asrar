...
// Réduit le HTML (sanitizé) en texte BRUT pour l'export Word. Les couleurs de
// l'aperçu (mots-clés en rouge/vert/bleu, versets intercalés en brun — voir
// appliquerCouleursManuscrit / formaterTexteIntercale, inchangés) aident à la
// lecture et au repérage dans l'app — notamment l'outil Intercaler, qui
// distingue visuellement les versets insérés — mais ne sont plus reportées
// dans le document exporté, qui reste entièrement en noir. Bonus : un texte
// sans changement de couleur tient dans un seul run par bloc (voir
// alqalamWorker.js) au lieu d'un run XML par mot coloré × répétition — bien
// plus rapide à assembler, quelle que soit la taille du document.
...
export async function generateDocx({ useOuv, useFerm, blocks, docName, fontPx = 28, onProgress = () => {} }) {
  ...
  const ouvText = useOuv ? htmlToPlainText(appliquerCouleursManuscrit(formules.ouverture, firstRasm)) : '';
  const fermText = useFerm ? htmlToPlainText(appliquerCouleursManuscrit(formules.fermeture, lastRasm)) : '';
  const blockPayload = blocks.map((block) => ({
    unitText: htmlToPlainText(appliquerCouleursManuscrit(block.texte, block.isRasmMode) + ' '),
